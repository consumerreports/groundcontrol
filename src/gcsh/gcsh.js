...
async function _testplan(subj_path, std_path, eval_id) {
    ...
    if (gs_api_idxs.length > 0) {
        ...
        if (input.toUpperCase() === "Y") {
            await app.data_modules[0].put("NEW", _testplan_to_gs_workbook(res));
        }
...
// exports of test structures (test results without the results yet) that are specific to Google Sheets. But we
// haven't yet figured out what the canonical form of our test structures is! For now, the ability to export 
// the result of the testplan function is essentially a prototype-era escape hatch provided by gcsh while we 
// figure out the right way for Ground Control to structure and store test results...
function _testplan_to_gs_workbook(tp) {
    const val = {
        properties: {
            title: "Hi i'm a test spreadsheet"
        }, 
